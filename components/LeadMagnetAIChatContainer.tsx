...
function LeadMagnetAIChatContainer({
  ...
  return (
    <div className="flex h-full max-w-3xl flex-col">
      <div className="grow space-y-4 overflow-y-auto rounded-md border-2 border-solid p-4">
        {messages.length === 0 && (
      ...
      <form onSubmit={handleSubmit} className="my-4 flex">
        <textarea
          ...
          className="grow rounded-md border border-gray-300 bg-white px-3 py-2 text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500"
        ...
        <button
          type="submit"
          className="ml-4 mt-auto h-10 shrink-0 rounded-md bg-purple-500 px-4 py-2 text-white disabled:cursor-not-allowed disabled:opacity-50"
          disabled={isLoading}
        >
          {isLoading ? (
